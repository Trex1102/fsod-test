...
EXP_NAME=voc_res5_adapter_ablations
BASE_EXP_NAME=vanilla_defrcn
SPLIT_ID=$1

if [ -z "${SPLIT_ID}" ]; then
    echo "Usage: bash run_voc_res5_adapter_ablations.sh <split_id>"
    exit 1
fi
...
ABLATIONS=${ABLATIONS:-"baseline block1_2 all_blocks heavy rpn_frozen"}
SHOTS=${SHOTS:-"1"}
SEEDS=${SEEDS:-"0"}
...
do
    ...
    do
        ABLATION_TEMPLATE_DIR=configs/voc/res5AdapterAblations/${ablation}
        if [ ! -d "${ABLATION_TEMPLATE_DIR}" ]; then
            echo "Missing ablation config directory: ${ABLATION_TEMPLATE_DIR}"
            exit 1
        fi

        SETTING_SAVE_DIR=${SAVE_DIR}/res5AdapterAblations/${ablation}/${setting}/split${SPLIT_ID}
        ...
        do
            ...
            do
                ...
                BASELINE_TEMPLATE=configs/voc/res5AdapterAblations/baseline/defrcn_${setting}_r101_novelx_${shot}shot_seedx_res5adapter.yaml
                BASELINE_CONFIG_PATH=configs/voc/res5AdapterAblations/baseline/defrcn_${setting}_r101_novel${SPLIT_ID}_${shot}shot_seed${seed}_res5adapter.yaml
