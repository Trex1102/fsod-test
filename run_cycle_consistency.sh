#!/usr/bin/env bash
# Direction 3: Cycle-Consistent Support Correspondence Runner
# =============================================================
# Runs cycle-consistent support correspondence (inference-only, no base training needed).
#
# Scores proposals by how well they mediate dense DINOv2 patch correspondences
# between multiple support exemplars. True objects produce low cycle error
# across support pairs s_i -> B -> s_j.
#
# This is eval-only and uses pretrained vanilla DeFRCN novel checkpoints.
#
# Usage:
#   bash run_cycle_consistency.sh <split_id> [shots] [seeds]
#
# Examples:
#   bash run_cycle_consistency.sh 1                           # All shots, seed 0
#   bash run_cycle_consistency.sh 1 "1 2 3 5 10" "0"         # All shots, seed 0
#   bash run_cycle_consistency.sh 1 "1 10" "0 1 2"           # 1+10 shot, 3 seeds
...
if [ -z "${SPLIT_ID}" ]; then
    echo "Usage: bash run_cycle_consistency.sh <split_id> [shots] [seeds]"
    echo ""
    echo "Examples:"
    echo "  bash run_cycle_consistency.sh 1"
    echo "  bash run_cycle_consistency.sh 1 \"1 2 3 5 10\" \"0\""
    echo "  bash run_cycle_consistency.sh 1 \"1 10\" \"0 1 2\""
    exit 1
fi
...
# Paths
# ======================================================================
EXP_NAME=voc_cycle_consistency
...
echo "=============================================="
echo "Direction 3: Cycle-Consistent Support Correspondence"
echo "=============================================="
echo "Split:   ${SPLIT_ID}"
echo "Shots:   ${SHOTS}"
echo "Seeds:   ${SEEDS}"
...
for shot in ${SHOTS}; do
    for seed in ${SEEDS}; do
        echo "  [cycle_consistency] ${shot}-shot, seed ${seed}"
        ...
        # Template config
        TEMPLATE_CONFIG=configs/voc/novelMethods/cycle_consistency/defrcn_fsod_r101_novelx_${shot}shot_seedx_cycle_consistency.yaml
        CONFIG_PATH=configs/voc/novelMethods/cycle_consistency/defrcn_fsod_r101_novel${SPLIT_ID}_${shot}shot_seed${seed}_cycle_consistency.yaml
...
echo ""
echo "=============================================="
echo "Cycle-Consistent Correspondence completed!"
echo "=============================================="
