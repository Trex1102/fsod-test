...
Examples:
  bash run_adapter_ablations.sh \
    --dataset coco --exp-name adapter_run1 --ablation baseline \
  ...
  bash run_adapter_ablations.sh \
    --dataset voc --split-id 1 --exp-name adapter_voc_s1 --ablation light \
    --imagenet-pretrain /data/.pretrain_weights/ImageNetPretrained/MSRA/R-101.pkl \
    --imagenet-pretrain-torch /data/.pretrain_weights/ImageNetPretrained/torchvision/resnet101-5d3b4d8f.pth \
...
CFG_ROOT="configs/${DATASET}/adapterAblations"
...
if [[ "${DATASET}" == "coco" ]]; then
  BASE_CFG="${ABLATION_ROOT}/defrcn_det_r101_base_adapter.yaml"
  MODEL_SURGERY_DATASET="coco"
  DATASET_FLAG="coco14"
  BASE_OUT_DIR="${SAVE_DIR}/defrcn_det_r101_base"
else
  BASE_CFG="${ABLATION_ROOT}/defrcn_det_r101_base${SPLIT_ID}_adapter.yaml"
...
TMP_CFG_ROOT="$(mktemp -d /tmp/adapter_ablation_cfgs.XXXXXX)"
cleanup() {
  rm -rf "${TMP_CFG_ROOT}"
}
trap cleanup EXIT
...
generate_seed_cfg() {
  ...
  local adapter_generated

  if [[ "${DATASET}" == "coco" ]]; then
    root_template="defrcn_${setting}_r101_novel_${shot}shot_seedx.yaml"
    adapter_template="defrcn_${setting}_r101_novel_${shot}shot_seedx_adapter.yaml"
    root_generated="defrcn_${setting}_r101_novel_${shot}shot_seed${seed}.yaml"
    adapter_generated="defrcn_${setting}_r101_novel_${shot}shot_seed${seed}_adapter.yaml"
  else
    root_template="defrcn_${setting}_r101_novelx_${shot}shot_seedx.yaml"
    adapter_template="defrcn_${setting}_r101_novelx_${shot}shot_seedx_adapter.yaml"
    root_generated="defrcn_${setting}_r101_novel${SPLIT_ID}_${shot}shot_seed${seed}.yaml"
    adapter_generated="defrcn_${setting}_r101_novel${SPLIT_ID}_${shot}shot_seed${seed}_adapter.yaml"
  fi
  ...
  local tmp_baseline_cfg="${tmp_dataset_root}/adapterAblations/baseline/${adapter_generated}"
  local tmp_ablation_cfg="${tmp_dataset_root}/adapterAblations/${ABLATION}/${adapter_generated}"

  render_template_cfg "${BASELINE_ROOT}/${adapter_template}" "${tmp_baseline_cfg}" "${seed}" "${DATASET}" "${SPLIT_ID}"
  ...
  render_template_cfg "${ABLATION_ROOT}/${adapter_template}" "${tmp_ablation_cfg}" "${seed}" "${DATASET}" "${SPLIT_ID}"
